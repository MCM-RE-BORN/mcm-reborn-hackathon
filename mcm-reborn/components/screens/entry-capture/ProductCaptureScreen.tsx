  ...
  if (state === "empty") {
    return (
      <StatusPanel
        ...
        description="분석 요청에는 정면, 후면, 상단, 하단, 좌측면, 우측면 JPG 또는 PNG 사진이 모두 필요해요. 시리얼 번호 사진은 선택 사항이에요."
  ...
  if (state === "error" || state === "limited") {
    return (
      <StatusPanel
        ...
        description={
          state === "limited"
            ? "정면, 후면, 상단, 하단, 좌측면, 우측면 사진을 각각 1장씩 등록해주세요. 파일당 최대 10MB의 JPG, PNG만 등록할 수 있어요."
  ...
  return (
    <AppShell
      ...
      header={
        <PageHeader
          backHref="/home"
          description="정면, 후면, 상단, 하단, 좌측면, 우측면 사진을 모두 선명하게 등록해주세요. 시리얼 번호 사진과 번호 입력은 선택 사항이에요."
          title="제품 사진 등록"
        />
      }
    >
