export const TEXTURE_PRIVACY_NOTICE_VERSION =
  "MCM_EXTERNAL_AI_ANALYSIS_TEXTURE_2026_08_21_R5" as const;

export const TEXTURE_PRIVACY_NOTICE_KO =
  "동의하면 AI 분석 접수 시 등록 사진 6장이 OpenAI로 전송되어 사진 품질, 소재·손상 상태와 외관의 본체·트림·스트랩·하드웨어를 함께 분석합니다. 이후 3D 목업 화면에서 사용자가 외관 목업 생성 버튼을 누르면 저장된 외관 분류를 재사용하고 외관 사진 4장이 Meshy로 전송되어 참고용 원제품 3D/PBR 및 여권 지갑용 목표 UV 텍스처를 생성할 수 있습니다. 저장된 외관 분류가 없는 데모 또는 폴백 분석에서는 버튼을 누를 때 외관 사진 4장이 OpenAI에 추가 전송될 수 있습니다. 이 버튼은 별도 재동의 없이 비용이 발생할 수 있는 외부 API 작업을 즉시 시작합니다. 안감은 처리하지 않습니다. OpenAI API 콘텐츠는 기본 악용 모니터링을 위해 최대 30일, Meshy API 자산은 최대 3일 보관될 수 있으며, 중복 과금 방지를 위해 파생 결과 또는 작업 참조가 서비스 서버에 캐시됩니다. 결과는 주문 전 시각화이며 실제 재단 위치나 완성품을 보증하지 않습니다.";
