...
- 전체 롤백은 `supabase/rollbacks/202608190005_shipment_conflict_hotfix.sql`부터 시작한 뒤 `supabase/rollbacks/202608180004_backend_v2_runtime.sql`을 적용합니다. 005 rollback은 배송 시작 결함을 다시 만들므로 앱도 함께 되돌릴 때만 사용합니다. 004 rollback은 migration 당시 private backup으로 제품 JSON·trigger·정책·권한을 복원하며, migration 뒤 해당 값이 바뀌었거나 실제 LIVE 동의 증적 행이 있으면 덮어쓰거나 증적을 삭제하지 않고 중단합니다.
