...
4. 실제 촬영 실패에 대비해 좌측면·우측면·하단·후면 JPG/PNG 예시 사진을 준비한다.
...
2. 모바일 카메라를 열어 좌측면 사진을 촬영하고 미리보기에서 `이 사진 사용`을 선택한다.
3. 우측면·하단·후면 사진을 차례로 추가해 필수 4슬롯을 모두 채운다.
...
- 사진 부족·형식·용량: JPG/JPEG·PNG, 좌측면·우측면·하단·후면 4장, 장당 10MB 기준과 남은 슬롯을 안내한다.
...
| 실패 | 즉시 복구 |
|---|---|
| 카메라 권한·장치 오류 | 파일 선택 폴백 또는 준비한 필수 구도 JPG/PNG 4장 사용 |
...
- [ ] 실제 카메라 또는 파일 폴백으로 좌측면·우측면·하단·후면 사진을 모두 등록했다.
...
- [ ] JPG/JPEG·PNG, 필수 구도 4장, 장당 10MB 규칙이 UI와 계약에 일치한다.
