...
3. 후면·상단·하단·좌측면·우측면·일련번호 사진을 차례로 추가해 필수 7슬롯을 모두 채운다.
...
7. 추천 화면의 `트래블·지갑·파우치·키링` 탭을 전환한다. 트래블의 `Ottomar 비세토스 여권 지갑` 카드만 선택해 RE:BORN 여권지갑 목업으로 이동하고, 별도 고정 목업 CTA가 없는지 확인한다.
...
4. 이 화면은 중앙 Fixture이며 실제 주문을 변경하지 않는다. live 연동 시에는 v2 lifecycle command의 운영자 JWT·멱등 키·인접 전이 검증을 사용한다.

## 대표 예외 시연

- 카메라 권한 거부: 권한 안내 뒤 `기기에서 사진 선택`으로 복구한다.
- 사진 부족·형식·용량: JPG/JPEG·PNG, 6면+일련번호 7장, 장당 10MB 기준과 남은 슬롯을 안내한다.
...
| 실패 | 즉시 복구 |
|---|---|
| 카메라 권한·장치 오류 | 파일 선택 폴백 또는 준비한 필수 구도 JPG/PNG 7장 사용 |
...
- [ ] 실제 카메라 또는 파일 폴백으로 정면·후면·상단·하단·좌측면·우측면·일련번호 사진을 모두 등록했다.
...
- [ ] JPG/JPEG·PNG, 필수 구도 7장, 장당 10MB 규칙이 UI와 계약에 일치한다.
