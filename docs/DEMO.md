...
- 기본 브라우저 데모는 외부 API를 호출하지 않고 `DEMO_FIXTURE`와 여권 지갑 전면 placeholder로 같은 주문과 보증서까지 진행한다. 통합 동의·키·배포 gate가 준비된 경우에만 사용자가 목업 화면 버튼을 눌러 OpenAI 외관 소재 계획과 Meshy source/target 작업을 선택적으로 시연한다. 생성 전·중·실패에는 placeholder를 유지하고 최종 텍스처 적용 뒤에만 3D를 공개한다.
...
선택형 OpenAI/Meshy 외관 목업의 키, 공개 GLB, 분석 접수 시 통합 동의, 보존 기간, quota와 비용 준비는 [`AI_TEXTURE_MOCKUP_PIPELINE.md`](./AI_TEXTURE_MOCKUP_PIPELINE.md)를 따른다. 단일 `외관 목업 생성` 버튼은 기본값 `ENABLE_TEXTURE_AI=false`에서 비활성이다. 분석 생성만으로 외관 작업을 자동 시작하지 않는다.
...
4. 카테고리 가방, 2019년, 5년 이상, 희망 제품 RE:BORN 여권지갑을 확인한다. OpenAI 분석과 이후 목업 버튼 클릭 시 OpenAI/Meshy 외관 생성·보존·비용성 호출을 함께 설명하는 R4 통합 안내에 동의한 뒤 `AI 분석 접수하기`를 누른다.
...
7. 추천 화면의 `트래블·지갑·파우치·키링` 탭을 전환한다. 트래블의 `여권 지갑` 카드만 선택해 RE:BORN 여권지갑 목업으로 이동하고 전면 placeholder가 표시되는지 확인한다. 외부 provider를 준비한 시연에서만 `외관 목업 생성`을 한 번 누른다. 화면에는 분석 결과·외관 4면·소재 분류·5단계 절차 대신 진행률과 최소 상태만 표시한다. 생성 텍스처가 스티치 보존 마스크를 거쳐 PNG로 합성되고 실제 3D material에 적용된 뒤에만 회전 가능한 3D가 나타나는지 확인하고, 이후 기본/맞춤 외관을 비교한다.
...
- OpenAI/Meshy 제공자 오류: 분석은 `DEMO_FIXTURE`로 복구하고 목업은 전면 placeholder를 유지한다. 유료 요청을 연속 클릭하지 않고 동일 job token으로 상태를 재확인하며, source 3D 실패는 target 작업 실패로 표현하지 않는다.
...
| OpenAI/Meshy 지연·오류 | 분석은 `DEMO_FIXTURE`, 목업은 전면 placeholder를 유지하고 동일 유료 job을 새로 만들지 않음 |
...
- [ ] 외부 texture 기능을 켠 경우 분석 접수의 R4 통합 동의, 버튼 시작, provider별 일회 생성과 Meshy 상태 복구를 확인했다.
