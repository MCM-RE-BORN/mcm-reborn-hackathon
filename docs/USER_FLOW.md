...
1. `/`의 1초 시작 화면에서 자동으로 상세 소개로 이동하고, `당신의 MCM에 새로운 여정을`과 6단계 안내를 확인한 뒤 고객으로 로그인한다. 로그인하면 같은 시작 화면에서 초기 고객 데이터가 준비된 뒤 홈의 실제 hero 영상과 `상품 진단하기` CTA를 확인한다.
2. 제품 등록에서 MCM 모노그램 백팩의 정면·후면·상단·하단·좌측면·우측면 사진을 모바일 카메라 또는 파일 선택으로 각각 1장씩 모두 등록한다. 시리얼 번호 사진은 선택이며, 촬영하면 데모 값 `MK123456789`가 자동 입력된다. 촬영하지 않으면 시리얼 번호를 직접 입력한다.
3. 제품 카테고리, 구매 연도 2019, 주요 사용 기간 5년 이상, 희망 제품 `RE:BORN 여권지갑`과 필수 시리얼 번호를 입력한다. 시리얼 번호는 ASCII 영문자와 숫자를 각각 하나 이상 포함하는 정확히 11자리여야 하며, 상태 메모만 선택이다.
...
- 업로드 형식·개수·크기가 JPG/JPEG·PNG, 필수 구도 6장, 장당 10MB 규칙과 다르면 입력을 보존하고 수정 방법을 안내한다.
