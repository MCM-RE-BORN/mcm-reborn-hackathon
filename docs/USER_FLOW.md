...
1. 서비스 소개를 확인하고 데모 고객으로 로그인하거나 홈으로 이동한다. 홈의 실제 hero 영상과 `상품 진단하기` CTA를 확인한다.
2. 제품 등록에서 MCM 비세토스 모노그램 백팩의 좌측면·우측면·하단·후면 사진을 모바일 카메라 또는 파일 선택으로 각각 1장씩 모두 등록한다.
...
7. 추천 화면에서 `트래블·지갑·파우치·키링` 제품군을 전환하고, 트래블의 `Ottomar 비세토스 여권 지갑`을 선택해 목업을 회전·확대 또는 정적 다각도 프레임으로 확인한다. 다른 상품 카드는 목업 상세로 진입하지 않는다.
...
10. 하단 `신청 내역`으로 `/orders` 목록을 열고 `RE:BORN 여권지갑`을 선택해 `/orders/demo` 상세로 이동한다.
...
- 업로드 형식·개수·크기가 JPG/JPEG·PNG, 필수 구도 4장, 장당 10MB 규칙과 다르면 입력을 보존하고 수정 방법을 안내한다.
