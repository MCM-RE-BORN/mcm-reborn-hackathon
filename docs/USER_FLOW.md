...
2. 제품 등록에서 MCM 모노그램 백팩의 정면·후면·상단·하단·좌측면·우측면 6면과 시리얼번호 사진을 모바일 카메라 또는 파일 선택으로 각각 1장씩 모두 등록한다. 시리얼번호 사진을 촬영하면 데모 값 `MK123456789`가 자동 입력되고 11자리 영문·숫자 인증을 통과한다.
