...
Meshy token은 고객, 분석, jobKind, provider task ID와 72시간 만료를 전용 `TEXTURE_TASK_SIGNING_SECRET` HMAC으로 서명한다. token은 암호문이 아니므로 URL query가 아니라 인증된 PUT body에만 보낸다. polling은 진행 중 Promise까지 합치는 짧은 in-process cache를 사용한다. provider 429는 새 task를 생성하지 않고 같은 token으로 재조회하며, `Retry-After`가 있으면 최대 60초까지 반영하고 없으면 3초를 사용한다. 연속 429는 20회로 제한하되 정상 상태 조회 80회에서는 차감하지 않는다. 이는 서버리스 인스턴스 간 분산 rate limit이나 durable queue를 대체하지 않는다.
