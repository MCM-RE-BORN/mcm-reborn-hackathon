...
7. 공식 편집·상품 페이지 DOM에서 이미지 URL을 찾고 자산 ID 기준으로 반응형 리사이즈·DOM 반복을 제거했다. 소재 SKU 이미지 83개는 2026-08-21에 공식 CDN 응답을 확인했다.
...
- 제품 사진과 원문 페이지를 로컬에 보관하지 않았으므로 미래 재현 시 URL이 사라질 수 있다. CDN 자산 공개 시각은 제품 출시일이 아니다.
