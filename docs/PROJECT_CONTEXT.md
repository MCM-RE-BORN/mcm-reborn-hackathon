...
- `LIVE` 분석 구현은 서버 전용 키와 공식 OpenAI JavaScript SDK의 Chat Completions Structured Outputs(`chat.completions.parse` + Zod)를 사용한다. 정면·후면·상단·하단·좌측면·우측면 6장 각각의 바로 앞에 `IMAGE_INDEX`와 `VIEW` 라벨을 넣는다. 한 요청에서 사진 품질·소재·상태·손상과 `BODY`/`TRIM`/`STRAP`/`HARDWARE` 외관 profile을 함께 생성하며 `BODY=PRESENT`가 없는 `ACCEPTABLE` 결과는 LIVE 성공으로 저장하지 않는다. `MCM_REUSE_GUIDE_2026_08_21_V1` PDF 정규화 규칙과 공개 KB `MCM_LEATHER_BAGS_PUBLIC_RESEARCH_2026_08_21_V1`의 선별 claim을 함께 grounding하되 사진 증거보다 우선하지 않는다. 공개 claim canonical SHA-256은 `1027b306a500b3f9b348f5e9db3489d65ba2114b1eb6d7ed4bb6920af07be03f`이며 결합 버전과 claim ID provenance를 분석 request hash와 private `provider_result`에 기록한다. profile과 지식 provenance는 목업 서버 내부 context이며 고객용 분석 응답에는 노출하지 않는다. 배포 opt-in과 `MCM_EXTERNAL_AI_ANALYSIS_TEXTURE_2026_08_21_R5` 통합 안내 동의가 있어야 한다. LIVE OpenAI 제공자 장애는 검증된 Fixture로 폴백할 수 있으며 그 결과의 `modeUsed`는 non-LIVE다. 실제 OpenAI 호출은 아직 검증되지 않았다.
- 입력과 AI 응답은 Zod로 검증한다. 목업 상세는 실제 여권지갑 GLB와 `@google/model-viewer` 런타임을 사용한다. 외관 작업은 분석 완료 시 자동 실행하지 않고 `외관 목업 생성` 버튼으로 시작한다. 현재 knowledge fingerprint의 LIVE profile이 있으면 `EXTERIOR_PLAN`은 저장값을 결정론적으로 변환하므로 외관 사진 signed URL, 추가 OpenAI 호출, 멱등 예약과 quota를 소비하지 않는다. `modeUsed=LIVE`인데 현재 profile이 없는 기존 분석은 409로 새 분석을 요구하며 classifier로 폴백하지 않는다. 4면 OpenAI `ExteriorMaterialClassifier`는 profile이 없는 non-LIVE `DEMO_FIXTURE`/`SEEDED_ESTIMATE` 호환 경로만 담당한다. 이후 정면·우측면·후면·좌측면 4장을 Meshy source 3D/PBR과 Meshy 7 목표 UV 리텍스처 job에 사용한다. texture service는 분석에 연결된 R5 통합 동의가 없으면 외부 호출 전에 403으로 거부한다. AI 계획의 semantic mask는 항상 unavailable이며 실제 배치는 사전 제작된 canonical target `exterior-mask`와 `stitch-preserve-mask`가 결정한다. 화면은 분석 narrative나 세부 절차를 반복하지 않고 버튼·진행률을 유지하며 생성 전·중·실패에 여권 지갑 전면 placeholder를 표시한다. 최종 base-color가 `exterior-mask * (1 - stitch-preserve-mask)`로 PNG 합성되고 canonical GLB에 실제 `applied`된 뒤에만 회전 가능한 3D와 기본/맞춤 비교를 공개한다. 원본 스티치 RGB와 기존 normal·metallic-roughness는 보존한다. source 3D는 참고용이고 안감은 MVP에서 제외한다. 외부 기능은 기본 비활성이고 실제 키를 사용한 staging 호출은 별도 검증 대상이다. DB의 Product3D 계약은 보존하며, 준비되지 않은 제품 자산은 계속 `model_3d_ready=false`로 API 노출을 막는다.
