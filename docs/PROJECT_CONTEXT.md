...
- 중앙 시나리오는 원제품 `MCM 모노그램 백팩(2019, 5년 이상)`, 희망 제품 `RE:BORN 여권지갑`, 접수 `SUB-RB-20260817-0001`, 주문 `RB-20260817-0001`을 사용한다.
...
- `LIVE` 분석 구현은 서버 전용 키와 공식 OpenAI JavaScript SDK의 Chat Completions 도구 호출·Structured Outputs(`chat.completions.parse` + Zod)를 사용한다. 정면·후면·상단·하단·좌측면·우측면 6장의 원래 `IMAGE_INDEX`와 `VIEW` 라벨을 고정한다. 첫 `detail: low` 호출은 `0 FRONT`, `1 REAR`, `4 LEFT`, `5 RIGHT` 외관 4면만 사용해 관찰 가능한 용어를 생성하고, 서버가 제품 예시와 비시각 레코드를 제외한 18-claim 위키를 결정론적으로 검색해 최대 5개·4,000자의 context를 만든다. 두 번째 `detail: auto` 호출은 상단·하단을 포함한 6면, PDF V1, 7개 always-on 금지 경계와 검색 context를 사용해 사진 품질·소재·상태·손상과 `BODY`/`TRIM`/`STRAP`/`HARDWARE` 외관 profile을 함께 생성한다. `BODY=PRESENT`가 없는 `ACCEPTABLE` 결과는 LIVE 성공으로 저장하지 않는다. lookup 실패·무결과는 전체 corpus가 아니라 always-on 경계로 보수적 폴백한다. 결합 지식 버전, retrieval corpus SHA, 검색기 버전, lookup 요청 ID·query/context hash와 실제 claim/source ID를 request hash 및 private `provider_result`에 기록한다. profile과 지식 provenance는 고객 응답에 노출하지 않는다. 배포 opt-in과 `MCM_EXTERNAL_AI_ANALYSIS_TEXTURE_2026_08_21_R5` 동의가 필요하며 LIVE 제공자 장애는 non-LIVE Fixture로 폴백할 수 있다.
- 입력과 AI 응답은 Zod로 검증한다. 목업 상세는 실제 여권지갑 GLB와 `@google/model-viewer` 런타임을 사용한다. 외관 작업은 분석 완료 시 자동 실행하지 않고 `3D 목업 생성` 버튼으로 시작한다. 현재 결합 지식 버전·retrieval corpus·검색기·trace가 일치하는 LIVE profile은 `EXTERIOR_PLAN`으로 결정론적 변환하므로 외관 사진 signed URL, 추가 OpenAI 호출, 멱등 예약과 quota를 소비하지 않는다. 현재 profile이 없는 기존 LIVE 분석은 409로 새 분석을 요구하며 classifier로 폴백하지 않는다. 4면 OpenAI `ExteriorMaterialClassifier`는 profile 없는 non-LIVE 호환 경로만 담당하고 공통 bounded OpenAI 재시도 정책을 사용한다. 이후 정면·우측면·후면·좌측면 4장을 Meshy source 3D/PBR과 Meshy 7 목표 UV 리텍스처 job에 사용한다. texture service는 분석에 연결된 R5 동의가 없으면 외부 호출 전에 403으로 거부한다. 실제 배치는 canonical target `exterior-mask`와 `stitch-preserve-mask`가 결정하며, 합성 base-color가 canonical GLB에 `applied`되면 맞춤 3D를 공개한다. target 생성·조회·자산 합성·적용이 최종 실패하거나 Meshy를 사용할 수 없으면 provider 상태를 유지한 채 canonical GLB의 내장 PBR·스티치 상태를 데모 3D 목업으로 공개한다. `기본 3D 모델과 비교`는 사용자 제공 Meshy 모델의 2K·182,361-triangle 웹 최적화 파생 GLB를 표시하고 `맞춤 외관 다시 적용`은 canonical GLB와 합성 atlas로 복귀한다. 전환은 provider 호출을 만들지 않고 비교 모델 로드 실패 시 맞춤 외관으로 자동 복귀한다. 원본 스티치 RGB와 normal·metallic-roughness는 보존하고 안감은 제외한다. 외부 기능은 기본 비활성이며 DB Product3D 계약은 유지한다.
- `TARGET_RETEXTURE`의 앱 UTC 일일 한도는 기본 비활성이다. `MESHY_TARGET_RETEXTURE_DAILY_LIMIT_PER_USER`와 `MESHY_TARGET_RETEXTURE_GLOBAL_DAILY_LIMIT`의 빈값·`0`은 비활성, `1`~`100`은 각각 고객·배포 전체 안전 한도로 해석한다. `SOURCE_MODEL` 1/2와 non-LIVE `EXTERIOR_PLAN` 3/5 고정 한도는 유지하며, Meshy의 실제 크레딧·요청 제한은 provider `402`·`429`가 권위 있다. 명시적 HTTP 거절은 예약을 해제해 `429`만 재시도할 수 있고 크레딧·인증·설정 오류는 `retryable: false`다. task polling의 429·네트워크·408·5xx는 같은 token으로 최소 3초·최대 60초 간격, 전체 20회까지 재조회한다. 생성 POST의 네트워크·`408`·`5xx`·잘못된 성공 응답은 접수 여부가 불명확해 terminal lock을 유지한다. quota와 분리한 recovery receipt가 접수된 task token을 복구하고 고객·분석·job binding 확인 뒤 현재 secret으로 재서명하며, 필수 target을 생성·저장한 뒤에만 선택형 source를 요청한다.
