...
- `LIVE` 분석 구현은 서버 전용 키와 공식 OpenAI JavaScript SDK의 Chat Completions Structured Outputs(`chat.completions.parse` + Zod)를 사용한다. 전달받은 짧은 소재·구성 부위·재사용 기준은 `MCM_REUSE_GUIDE_2026_08_21_V1` developer prompt grounding으로 유지한다. 공개 조사 지식 `MCM_LEATHER_BAGS_PUBLIC_RESEARCH_2026_08_21_V1`은 전체 프롬프트가 아니라 서버 전용 로컬 위키로 두고, 첫 호출의 강제 검색 도구 → 최대 6개·8,000자 결과 → 최종 Structured Output의 두 호출로 조회한다. 검색 코퍼스·검색기·프롬프트 버전과 조회 레코드·출처·컨텍스트 해시를 내부 `provider_result`에 감사 기록하며 사진 증거보다 우선하지 않는다. 참조 전용 공식 이미지 URL은 외부 분석 입력으로 보내지 않는다. 배포 opt-in과 `MCM_EXTERNAL_AI_ANALYSIS_TEXTURE_2026_08_21_R4` 통합 안내 동의가 있어야 하며, 제공자 장애에는 검증된 Fixture로 폴백한다. 동의는 분석 접수에서 OpenAI 분석과 이후 사용자가 목업 버튼을 눌렀을 때 시작되는 OpenAI/Meshy 외관 작업을 함께 고지한다. 실제 OpenAI 호출은 아직 검증되지 않았다.
- 입력과 AI 응답은 Zod로 검증한다. 목업 상세는 실제 여권지갑 GLB와 `@google/model-viewer` 런타임을 사용한다. 외관 작업은 분석 완료 시 자동 실행하지 않고 `외관 목업 생성` 버튼을 누를 때 서버 저장 외관 4면으로 OpenAI BODY/TRIM/STRAP/HARDWARE 소재 계획, 선택형 Meshy source 3D/PBR과 Meshy 7 목표 UV 리텍스처를 별도 job으로 실행한다. texture service는 분석에 연결된 R4 통합 동의가 없으면 외부 호출 전에 403으로 거부한다. 화면은 분석 결과나 세부 절차를 반복하지 않고 생성 전·중·실패에 여권 지갑 전면 placeholder를 유지하며, 최종 base-color가 `exterior-mask * (1 - stitch-preserve-mask)`로 합성되어 canonical GLB에 실제 적용된 뒤에만 3D를 공개한다. 합성 결과는 PNG로 유지해 원본 스티치 RGB의 JPEG 재압축 변형을 막고, 기존 normal·metallic-roughness도 보존한다. source 3D는 참고용이고 안감은 MVP에서 제외한다. 외부 기능은 기본 비활성이고 실제 키를 사용한 staging 호출은 별도 검증 대상이다. DB의 Product3D 계약은 보존하며, 준비되지 않은 제품 자산은 계속 `model_3d_ready=false`로 API 노출을 막는다.
