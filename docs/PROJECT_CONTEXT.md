...
- 입력과 AI 응답은 Zod로 검증한다. 목업 상세는 실제 여권지갑 GLB와 `@google/model-viewer` 런타임을 사용한다. 외관 작업은 분석 완료 시 자동 실행하지 않고 `외관 목업 생성` 버튼으로 시작한다. 현재 결합 지식 버전·retrieval corpus·검색기·trace가 일치하는 LIVE profile은 `EXTERIOR_PLAN`으로 결정론적 변환하므로 목업 단계에서 추가 OpenAI 호출·quota를 소비하지 않는다. 현재 profile이 없는 기존 LIVE 분석은 409로 새 분석을 요구하며 classifier로 폴백하지 않는다. 4면 `ExteriorMaterialClassifier`는 profile 없는 non-LIVE 호환 경로만 담당한다. 이후 Meshy는 외관 4장을 사용하고, `exterior-mask * (1 - stitch-preserve-mask)` PNG 합성 결과가 canonical GLB에 실제 `applied`된 뒤에만 3D와 기본/맞춤 비교를 공개한다. 원본 스티치 RGB와 normal·metallic-roughness는 보존하고 안감은 제외한다. 외부 기능은 기본 비활성이며 DB Product3D 계약은 유지한다.
