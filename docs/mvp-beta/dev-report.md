...
- 원제품은 `MCM 모노그램 백팩(2019, 5년 이상)`, 희망 제품은 `RE:BORN 여권지갑`으로 통일한다.
- 접수 `SUB-RB-20260817-0001`, 주문 `RB-20260817-0001`, 보증서 `ESG-RB-20260817-0001`을 하나의 중앙 시나리오로 연결한다.
- 제품 등록은 정면·후면·상단·하단·좌측면·우측면 JPG/JPEG·PNG 사진 6장이 모두 필수이며 장당 최대 10MB다. 시리얼 번호 촬영은 자동 입력용 선택 기능이다.
...
- 홈 영상에 포함된 모바일 OS 상태바는 crop하며, `/operations`는 PC Fixture 목록·상세와 관리자·장인 다음 단계 진행을 제공한다.
...
브라우저에서는 실제 모바일 필수 구도 촬영 6장, 선택 시리얼 자동 입력, 카메라 왕복 상태 보존, 72%·91% 예상치, Mock 주문, 수거 후 변경 승인, PC 운영 콘솔, 68% 보증서까지 한 주문으로 수동 검증한다.
