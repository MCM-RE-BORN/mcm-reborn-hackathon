...
- 제품 등록은 정면·후면·상단·하단·좌측면·우측면·일련번호 JPG/JPEG·PNG 사진 7장이 모두 필수이며 장당 최대 10MB다.
...
브라우저에서는 실제 모바일 필수 구도 촬영 7장, 카메라 왕복 상태 보존, 72%·91% 예상치, Mock 주문, 수거 후 변경 승인, PC 운영 콘솔, 68% 보증서까지 한 주문으로 수동 검증한다.
