...
| 신청 내역 목록 | 하단 nav→신청/진단 탭→항목 선택 | n39 진입을 목록으로 보완 | `220:63`의 `신청 내역` nav | 부분 | 와이어프레임 + 디자인 스타일 | `/orders`; 최상위 화면이라 뒤로가기 없음, `신청 내역`·`진단 내역` 탭 분리, 진단은 결과 상세로·상품은 `/orders/demo`으로 이동, 완료 신청은 ESG 인증서 바로가기 표시 |
...
| 마이페이지 | 공통 하단 nav→프로필 | 최신 사용자 지시로 명시 | `228:1176`; nav `220:63` | 디자인 완료 | Figma대로 | `/mypage`; 최상위 화면이라 뒤로가기 없음, 이름/휴대폰 라벨 오류 교정 |
