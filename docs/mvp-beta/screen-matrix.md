...
| 제품 등록·촬영 업로드 | 홈→제품 사진/정보 제출 | n13, 완성 | `228:651` | 부분 | 와이어프레임 + 디자인 스타일 | `/products/new`; 정면·후면·상단·하단·좌측면·우측면 필수 6슬롯, JPG/PNG 6장·10MB, 필수 11자리 시리얼 텍스트와 선택 촬영 후 데모 텍스트 자동 입력·인증, 제품정보 세션 보존을 결합 |
...
- 제품 등록: 필수 6개 슬롯을 정면·후면·상단·하단·좌측면·우측면으로 두고 10MB/JPG·PNG 규칙과 제품 정보 보존을 결합한다. 시리얼 텍스트는 11자리 영문·숫자로 필수 입력하며, 선택적인 전용 시리얼 촬영 행은 입력란 아래에 배치하고 촬영 후 데모 번호를 자동 입력한다.
