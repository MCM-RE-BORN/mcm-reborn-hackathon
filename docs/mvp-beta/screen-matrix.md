...
| 홈 | 인증/게스트→주요 진입 | n11, 완성 | `220:63`; 영상 그룹 `220:65`; CTA `222:81` | 디자인 완료 | Figma대로 | `/home`; 실제 hero 영상 재생, `상품 진단하기` 공통 CTA, `신청 내역`·`홈`·`마이페이지` 3탭 내비게이션 |
| 제품 등록·촬영 업로드 | 홈→제품 사진/정보 제출 | n13, 완성 | `228:651` | 부분 | 와이어프레임 + 디자인 스타일 | `/products/new`; 좌측면·우측면·하단·후면 필수 4슬롯, JPG/PNG 4장·10MB, 제품정보 세션 보존을 결합 |
...
| 디자인 추천 | 분석→제품군 비교→선택 | n25, 완성 | `228:782` | 부분 | Figma대로 | `/submissions/demo/designs`; 트래블·지갑·파우치·키링 탭, 트래블의 Ottomar 비세토스 여권 지갑만 상세 진입, 고정 목업 CTA 없음 |
...
| 신청 내역 목록 | 하단 nav→신청 상품 선택 | n39 진입을 목록으로 보완 | `220:63`의 `신청 내역` nav | 부분 | 와이어프레임 + 디자인 스타일 | `/orders`; 중앙 Fixture 신청 상품 목록, 상품 선택 시 `/orders/demo`으로 이동 |
...
| 마이페이지 | 공통 하단 nav→프로필 | 최신 사용자 지시로 명시 | `228:1176`; nav `220:63` | 디자인 완료 | Figma대로 | `/mypage`; 이름/휴대폰 라벨 오류 교정, 실제 설정 저장 제외 |
...
- 홈: `220:65`의 실제 영상을 muted·loop·inline으로 재생하고 `222:81`의 `상품 진단하기` CTA를 적용한다. 상품 진단과 인증서 CTA는 공통 ActionButton을 사용하며 정적 이미지는 영상 재생 실패 시 fallback으로만 사용한다.
- 제품 등록: 4개 슬롯을 좌측면·우측면·하단·후면으로 두고 4장 모두 필수인 10MB/JPG·PNG 규칙과 제품 정보 보존을 결합한다.
