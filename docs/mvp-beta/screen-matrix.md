...
| 신청 내역 목록 | 하단 nav→AI 분석 결과 또는 신청 상품 선택 | n39 진입을 목록으로 보완 | `220:63`의 `신청 내역` nav | 부분 | 와이어프레임 + 디자인 스타일 | `/orders`; DB의 완료된 AI 분석 결과와 신청 상품 목록, 분석은 결과 상세로·상품은 `/orders/demo`으로 이동 |
