...
> 2026-08-18 화면의 최종 시각 기준은 Figma `디자인` 파일의 `최종 디자인` 페이지(`228:391`)다. 제품 수치·필드·상태 순서가 Figma와 충돌하면 [`../MVP_DEMO_CANONICAL.md`](../MVP_DEMO_CANONICAL.md)를 우선하고 시각 위계·간격·자산만 Figma를 따른다. 이전의 정적 진입점·1~4장/6MB/WebP·주문 전 수동 검토·별도 완료 주문 설명은 **superseded/historical**이다.
...
|---|---|---|---|---|---|---|
| 서비스 소개 | 최초 실행→소개→시작 | n2, 완성 | `228:769` 시작 화면 | 부분 | Figma대로 | `/`는 풀블리드 시작 화면, `/intro`는 가치·대상·6단계와 면책을 보완한 상세 소개 |
| 로그인 | 비회원 인증 | n6, 완성 | `228:1457` | 디자인 완료 | Figma대로 | `/login`; 데모 폼, 실제 인증 제외 |
| 회원가입 | 로그인→가입→홈 | n7, 완성 | `228:1510` | 부분 | Figma대로 | `/signup`; 계약 없는 저장 동작 제외, 약관 UI 포함 |
| 홈 | 인증/게스트→주요 진입 | n11, 완성 | `228:496` | 디자인 완료 | Figma대로 | `/home`; 검정→흰색 hero, 카피·CTA와 4탭 canonical navigation 적용 |
| 제품 등록·촬영 업로드 | 홈→제품 사진/정보 제출 | n13, 완성 | `228:651` | 부분 | 와이어프레임 + 디자인 스타일 | `/products/new`; Figma 사진 위계에 정면·측면·내부·각인, JPG/PNG 3~4장·10MB, 제품정보 세션 보존을 결합 |
...
| 디자인 추천 | 분석→후보 비교→선택 | n25, 완성 | `228:782` | 충돌 | 와이어프레임 + 디자인 스타일 | `/submissions/demo/designs`; 중앙 시나리오의 RE:BORN 여권지갑을 골든 후보로 사용, 추천 0건 상태 유지 |
...
| 주문 진행 조회·신청 내역 | 주문 완료→수거/검수/제작/배송 | n39, 완성 | `228:1086` | 부분 | Figma대로 | `/orders/demo`; 한 주문의 변경 전후 조건과 상태 타임라인 |
...
| 마이페이지 | 홈 하단 nav→프로필 | 최신 flow에는 명시 없음 | `228:1176` | 충돌 | Figma대로 | `/mypage`; 이름/휴대폰 라벨 오류 교정, 실제 설정 저장 제외 |
