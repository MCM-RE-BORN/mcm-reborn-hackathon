...
1. 사용자의 2026-08-18 최신 UI 지시
   ...
   - 촬영은 정면·후면·상단·하단·좌측면·우측면 6면과 시리얼번호 사진까지 총 7장을 받는다.
...
- 필수 슬롯: `정면`, `후면`, `상단`, `하단`, `좌측면`, `우측면`, `시리얼번호`
- 슬롯별 1장, 총 7장 모두 필수
- 허용 형식: JPG/JPEG, PNG
- 파일당 최대 10MB
- 모바일에서는 후면 카메라를 우선 열고 촬영 후 미리보기·재촬영·사용 선택을 제공한다.
- 권한 거부·카메라 미지원·장치 없음에는 앨범/기기 파일 선택 폴백을 제공한다.
- 시리얼번호 사진은 필수 슬롯이다. 촬영을 완료하면 데모 시리얼 `MK123456789`가 입력란에 자동 반영되며, 직접 입력한 값도 영문과 숫자를 포함한 11자리면 UI 인증을 통과한다.
...
- 모바일 실제 카메라 또는 파일 폴백으로 정면·후면·상단·하단·좌측면·우측면·시리얼번호 사진을 모두 등록하고, 시리얼번호 촬영 후 데모 텍스트를 자동 입력할 수 있다.
