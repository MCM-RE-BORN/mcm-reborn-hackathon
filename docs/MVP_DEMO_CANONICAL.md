...
1. 사용자의 2026-08-18 최신 UI 지시
   ...
   - 분석 촬영은 정면·후면·상단·하단·좌측면·우측면 6장을 필수로 받는다. 시리얼 번호 촬영은 텍스트 자동 입력을 위한 선택 기능이다.
...
- 시리얼 번호 사진은 선택이며 분석 자산에는 포함하지 않는다. `시리얼 번호 촬영하여 입력`을 사용하면 데모 시리얼 `MK123456789`가 입력란에 자동 반영되며, 직접 입력한 값도 영문과 숫자를 포함한 11자리면 UI 인증을 통과한다.
...
| 제품 시리얼 번호 | API 선택·UI 자동 입력 | 촬영 전 빈 값, 촬영 후 `MK123456789` |
