...
2. `MCM RE_BORN MVP_유저플로우_2026-08-13.md`의 화면·분기 순서
3. `와이어프레임.html`의 화면 필드·CTA·콘텐츠 구조
4. `MCM RE_BORN MVP_PRD_2026-08-13.md`와 기능명세서의 업무 의미·데이터 요구
5. 저장소의 OpenAPI·Mock·DB·기존 문서와 구현
...
flowchart TD
  ...
  D -->|골든 패스| E["사진 기반 AI 예상 결과"]
  E --> F["디자인 추천"]
  F --> G["RE:BORN 여권지갑 목업 확인"]
  G --> H["수거 정보 입력"]
  H --> I["주문 확인·Mock 결제"]
  I --> J["주문 완료"]
  J --> K["수거 예정·수거 완료"]
...
- 분석 중 화면은 1~2초간 보여 준 뒤 같은 접수 번호의 결과로 이동한다.
...
- 모바일 실제 카메라 또는 파일 폴백으로 사진 3장 이상을 등록할 수 있다.
