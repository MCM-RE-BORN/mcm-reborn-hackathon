...
1. 사용자의 2026-08-18 최신 UI 지시
   ...
   - 촬영은 정면·후면·상단·하단·좌측면·우측면 6면과 일련번호 사진까지 총 7장을 받는다.
   - 홈 영상에 포함된 모바일 OS 상태바는 노출하지 않고, PC 운영 콘솔은 목록·상세·다음 단계 진행만 제공한다.
...
- 공통 하단 내비게이션은 `신청 내역`(`/orders`)·`홈`(`/home`)·`마이페이지`(`/mypage`) 3개다. `/orders`는 완료된 AI 분석 결과와 주문 신청 상품을 함께 보여 주며, 분석 항목은 분석 상세로, 신청 상품은 `/orders/demo` 상세로 이동한다.
...
PC 운영 콘솔은 별도 상태 머신 없이 각 단계의 현재 담당을 관리자 또는 장인으로 표시한다. 관리자는 수거·배송 운영 단계를, 장인은 입고 후 검수·제작·품질 단계를 담당한다. 현재는 중앙 Fixture로만 진행하며 실제 역할 분리와 실데이터 쓰기는 후속 연동이다.
...
- 추천 화면은 `트래블`, `지갑`, `파우치`, `키링` 제품군을 전환한다. 골든 경로에서는 트래블의 `여권 지갑` 카드로 목업 상세에 진입한다.
...
- 모바일 실제 카메라 또는 파일 폴백으로 정면·후면·상단·하단·좌측면·우측면·일련번호 사진을 모두 등록할 수 있다.
...
- PC 운영 콘솔에서 신청 목록을 선택하고 현재 담당자가 수행할 허용된 다음 단계로 진행할 수 있다.

## 10. 범위 밖

- 실제 결제사·택배사·정품 인증 기관·탄소 검증 기관 연동
- 실제 장인용 독립 계정·배정·생산 운영 데이터 연동
