...
1. 사용자의 2026-08-18 최신 UI 지시
   ...
   - 분석 촬영은 정면·후면·상단·하단·좌측면·우측면 6장을 필수로 받는다. 시리얼 번호 촬영은 텍스트 자동 입력을 위한 선택 기능이며, 시리얼 번호 텍스트 자체는 신규 분석 접수에 필수다.
...
- `/`는 대표 이미지 준비 뒤 최소 1초 동안 시작 화면을 보여 준 다음 `/intro`로 자동 이동하고, `/intro`는 `새로운 여정`과 6단계 소개·스크롤 안내를 렌더한다. 고객 로그인 직후에는 `/` 시작 화면을 프로필·신청·진단 초기 로딩에 재사용한 뒤 `/home`으로 이동한다.
- 공통 하단 내비게이션은 `신청 내역`(`/orders`)·`홈`(`/home`)·`마이페이지`(`/mypage`) 3개다. 신청 내역과 마이페이지에는 뒤로가기를 표시하지 않는다. `/orders`는 `신청 내역`과 `진단 내역` 탭을 분리하며, 진단 항목은 분석 상세로, 신청 상품은 `/orders/demo` 상세로 이동하고 완료 신청에는 발급된 ESG 인증서 바로가기를 표시한다.
- 하위 화면의 뒤로가기는 공통 `PageHeader`의 안전영역 아래 동일한 좌측 위치와 44px 터치 영역을 사용한다. 몰입형 카메라의 오버레이 버튼도 같은 좌표를 따른다.
...
- 고객 인증·분석·추천·신청·결제·검수·보증서 브라우저 화면은 v2 Route Handler와 Supabase 응답을 사용한다. 분석 서버의 `DEMO_FIXTURE` 모드는 재현 가능한 AI 예상값만 제공하며 인증·빈 결과·API 오류를 임의 고객 데이터로 대체하지 않는다. 신청 내역과 마이페이지는 로그인 초기 로딩의 사용자별 메모리 캐시를 즉시 표시하고 화면 진입 시 조용히 재검증한다. 실제 Supabase/OpenAI 전체 원격 여정은 별도 staging 검증 대상이다.
...
- 필수 슬롯: `정면`, `후면`, `상단`, `하단`, `좌측면`, `우측면`
- 슬롯별 1장, 총 6장 모두 필수
- 허용 형식: JPG/JPEG, PNG
- 파일당 최대 10MB
- 모바일에서는 후면 카메라를 우선 열고 촬영 후 미리보기·재촬영·사용 선택을 제공한다.
- 권한 거부·카메라 미지원·장치 없음에는 앨범/기기 파일 선택 폴백을 제공한다.
- 시리얼 번호 사진은 선택이며 분석 자산에는 포함하지 않는다. `시리얼 번호 촬영하여 입력`을 사용하면 데모 시리얼 `MK123456789`가 입력란에 자동 반영된다. 시리얼 번호 텍스트는 신규 분석 접수에 필수이며, 앞뒤 공백을 제외하고 ASCII 영문자와 숫자를 각각 하나 이상 포함하는 정확히 11자리여야 한다.
...
| 제품 시리얼 번호 | 필수·촬영 시 자동 입력 | 직접 입력 또는 촬영 후 `MK123456789` |
...
- 모바일 실제 카메라 또는 파일 폴백으로 정면·후면·상단·하단·좌측면·우측면 사진 6장을 모두 등록하고, 선택적인 시리얼 번호 촬영 후 데모 텍스트를 자동 입력할 수 있다.
